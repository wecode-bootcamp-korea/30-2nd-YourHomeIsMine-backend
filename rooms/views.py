...
class RoomListView(View):
    def get(self, request):
        ...
        results = [{
            ...
            'schedule'      : 
                datetime.strftime([room_schedule.check_in for room_schedule in room.room_schedules.all()][0], '%m월 %d일').replace('0','')+" ~ "+
                datetime.strftime([room_schedule.check_in for room_schedule in room.room_schedules.all()][-1], '%m월 %d일').replace('0',''),
            'price'         : int(room.price),
            'images'        : [image.image_url for image in room.room_images.all()],
            'latitude'      : float(room.latitute),
            'longitude'     : float(room.longitute),
            'month'         : datetime.strftime([room_schedule.check_in for room_schedule in room.room_schedules.all()][0], '%m').replace('0',''),
            'schedule_date' : 
                datetime.strftime([room_schedule.check_in for room_schedule in room.room_schedules.all()][0], '%Y%m%d')+","+
                datetime.strftime([room_schedule.check_in for room_schedule in room.room_schedules.all()][-1], '%Y%m%d'),
        } for room in rooms]
