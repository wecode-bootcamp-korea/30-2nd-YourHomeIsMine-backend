...
class ReviewView(View):   
    def get(self, request, room_id):
        try:  
            reviews = Room.objects.get(id=room_id).reviews.all()
            average = reviews.aggregate(Avg('accuracy'), Avg('communication'), Avg('cleanliness'), Avg('location'), Avg('check_in'), Avg('cost_performance'))
            result  = {
                ...
                'check_in'        : round(average['check_in__avg'], 2),
                ...
                'reviews'         : [
                    {
                        ...
                        'review_image'  : [review_image.image_url for review_image in review.review_images.all()]
